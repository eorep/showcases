...
In this report I aim to show the 53 locations where pedestrians in the city of Montreal can use the streets as sidewalks. The data was obtained from the open dataset initiative of the city of Montreal.
...
- Generated a url field that contains the link to the photo when available.
- Created a color field to match the implantation mode (permanent, temporary or seasonal setting).
...
data <- data %>%
  ...
  mutate(url = ifelse(photo2 != "", paste0("<a href='",photo2,"'>",TOPONYME,"</a>"), as.character(TOPONYME))) %>%
...
- The color of the circles indicate the type of location: permanent, temporary or seasonal. There is a legend on the top right that indicate the different types.
- The size of the circles represent the related size of each location.
- Clicking on each circle will open a pop up that indicate the name of the location. 
- In case a link is available, the name of the location will become a link.  Please note that in some cases the links are returning an error (not found in the server). 
...
The map shows the 53 locations where pedestrians in Montreal can walk in the street by type (permanent, temporary or seasonal location) and relative size. In case where the information is available, a photo of the location can be accessed by clicking in the respective circle and the link inside. 
...
- Document created on May 8th, 2020.
